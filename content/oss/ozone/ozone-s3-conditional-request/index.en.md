...
summary: "How Apache Ozone implements S3 conditional requests with its atomic rewrite path, enabling fast compare-and-swap style coordination for object-storage-backed systems."
description: "Apache Ozone is adding S3 conditional request support for PutObject, GetObject, HeadObject, CopyObject, DeleteObject, and CompleteMultipartUpload. This post explains the use cases, implementation design, and benchmark results for fast optimistic concurrency control on Ozone S3."
date: 2026-07-08T01:00:00+08:00
slug: "ozone-s3-conditional-request"
tags: ["blog", "en", "oss", "ozone", "s3", "concurrency"]
cascade:
  showEdit: true
  showSummary: true
...
![Dynamic model reloading with S3 ETag validation](model-reload.png "Figure 1. An inference server uses `If-None-Match` to avoid re-downloading unchanged model weights.")
...
![Object-storage queue coordinated through compare-and-swap updates](object-queue.png "Figure 2. Turbopuffer’s object-storage queue batches contended CAS updates behind a broker. Source: [Turbopuffer object storage queue](https://turbopuffer.com/blog/object-storage-queue).")
...
![SlateDB writer claiming the next object with create-if-absent semantics](slatedb-writer.png "Figure 3. A SlateDB writer uses conditional object creation as a fencing point for object-store state.")
...
![Iceberg catalog pointer update protected by ETag comparison](iceberg-catalog.png "Figure 4. An Iceberg-style file catalog updates a small metadata pointer only if its ETag still matches.")
...
![Leader election through conditional lock-file creation](leader-election.png "Figure 5. Nodes compete to create the next epoch lock file; only one writer succeeds.")
...
![Atomic rewrite create and write phases](atomic-rewrite-create.png "Figure 8. Atomic rewrite create and write phases.")
...
{{< mermaid caption="Figure 12. `PUT If-Match` validates the ETag once, stores the matched epoch, and rechecks that epoch at commit time." >}}
sequenceDiagram
    actor User
    participant GW as S3 Gateway
    participant OM as Ozone Manager
...
{{< mermaid caption="Figure 13. `GET` and `HEAD` evaluate conditional headers after retrieving object metadata from OM." >}}
sequenceDiagram
    actor User
    participant GW as S3 Gateway
    participant OM as Ozone Manager
...
{{< mermaid caption="Figure 14. `CopyObject` checks source preconditions first, then reuses conditional destination writes." >}}
sequenceDiagram
    actor User
    participant GW as S3 Gateway
    participant OM as Ozone Manager
...
{{< mermaid caption="Figure 15. Conditional `CompleteMultipartUpload` validates the destination state before publishing the completed object." >}}
sequenceDiagram
    actor User
    participant GW as S3 Gateway
    participant OM as Ozone Manager
...
{{< mermaid caption="Figure 16. Conditional `DeleteObject` deletes only when the current object still matches the expected ETag." >}}
sequenceDiagram
    actor User
    participant GW as S3 Gateway
    participant OM as Ozone Manager
...
![Benchmark comparing normal and conditional PUT latency](benchmark-latency.png "Figure 17. Local benchmark latency for normal `PUT`, `PUT If-None-Match`, normal overwrite, and `PUT If-Match`.")
